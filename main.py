...
def search(data, anno, chromosome):
    ...
    if ind:
        for i in ind:
            beg.append(chromosome + '\t' + str(start) + '\t' + str(a.loc[i]['gene']) + '\t' + str(data[0]-int(a.loc[i]['pos1'])))
    else:
    ...
    if ind:
        for i in ind:
            end.append(chromosome + '\t' + str(fin) + '\t' + str(a.loc[i]['gene']) + '\t' + str(data[1]-int(a.loc[i]['pos1'])))
    else:
...
if __name__ == "__main__":
    # read file
    depth = pd.read_csv(r"D:\annotation\DeepSeq\data\depth.txt", sep='\t', names=['chr', 'pos', 'cov'])
    info = pd.read_csv('data\\gene_range', sep='\t', names=['gene', 'chr', 'pos1', 'pos2'])
    # del < 300
    ...
    #output
    with open('data\\anno.txt','w+') as f:
        for i in out:
            for j in i[0]:
                f.write('{}\t'.format(j))
            f.write('\t~\t')
